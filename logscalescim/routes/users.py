...
from ..graphql_client import LOGSCALE_GQL_MUTATION_USER_ADD, LOGSCALE_GQL_QUERY_USER_BY_KEY, LOGSCALE_GQL_MUTATION_USER_UPDATE_BY_ID
...
bp = Blueprint('Users', __name__,
               url_prefix=f'{current_app.config['REQUEST_PATH_PREFIX']}/Users')
...
def create_user():
    ...
    try:
        ...
        id = None
        if existingId:
            logger.warning("User already exists.", extra={
                           "username": data["userName"], "email": primary_email, "externalId": data["externalId"], "logscaleId": existingId})
            variables = {
                "input": {
                    "userId": existingId,
